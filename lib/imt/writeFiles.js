...
function moveFiles(folderPath, insFileName, dataFileName, outputFileName, resFileName, callback) {
    mv('./' + insFileName, folderPath + '/'+ insFileName, function(err) {
        if (err) throw err;
        mv('./' + dataFileName, folderPath + '/'+ dataFileName, function(err) {
            if (err) throw err;
            mv('./' + outputFileName, folderPath + '/'+ outputFileName, function(err) {
                if (err) throw err;
                mv('./' + resFileName, folderPath + '/'+ resFileName, function(err) {
                    if (err) throw err;
                    callback();
                });
            });
